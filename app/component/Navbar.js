  ...
  return (
    <header className="bg-black py-3">
      <nav className="flex justify-between items-center w-[92%] mx-auto">
        ...
        <div
          className={`md:static md:min-h-fit absolute bg-black py-5 min-h-[35vh] left-0 ${
            open ? `top-[8%]` : `top-[-100%]`
          } md:w-auto w-full  flex items-center px-5 md:z-[0] z-[2] transition-all duration-500 ease-linear`}
        >
          <ul className="flex md:flex-row flex-col mx-auto items-center md:gap-[4vw] gap-8">
            {navlist.map((lst, ind) => {
              return (
                <li key={ind}>
                  <Link
                    className=" text-white hover:bar text-lg"
                    href={lst.link}
                  >
                    {lst.name}
                  </Link>
